...
public class Terrain2D: MonoBehaviour
{
	...
	{
		brushsize_radius,
		distance_proportional_radius,
		single_cell_hard,
		single_cell_soft,
		multi_cell_soft,
		distance_single_or_multi_cell_auto,
		smooth_brushsize_radius,
		smooth_distance_proportional_radius,
		smooth_single_cell_hard,
		smooth_single_cell_soft,
		smooth_multi_cell_soft,
		smooth_distance_single_or_multi_cell_auto }
	;
	public enum TerrainLayer2D
	{
		master,
		material1,
		material2,
	...
	[HideInInspector]
	public int dataTop = 0;
	[HideInInspector]
	public float[] datad = null;
	...
	[HideInInspector]
	public int dataMaterialLayers = 4;
	...
	[HideInInspector]
	public float[] waterData = null;
	public float waterDataChange = 0.1f;
	[HideInInspector]
	public float waterBias = 1.0f;
	[HideInInspector]
	public float waterDelta = 1.0f;
	...
	public class TerrainBlock2D
	{
		...
		[System.NonSerialized]
		public int count = 1999999999;
		[System.NonSerialized]
		public float lastRedrawTime = -999999999f;
	...
	[System.NonSerialized]
	public TerrainBlock2D[] blocks = null;
	[HideInInspector]
	public int blocksLeft = 0;
	[HideInInspector]
	public int blocksTop = 0;
	[HideInInspector]
	public int blocksWidth = 0;
	[HideInInspector]
	public int blocksHeight = 0;
	...
	private bool keynamesGetKey (string keynames)
	{
		...
		for (int i = 0; i < keynamesArray.Length; ++i) {
			try {
				if (Input.GetKey (keynamesArray [i]))
					return true;
			} catch {
				;
			}
	...
	private bool keynamesGetKeyDown (string keynames)
	{
		...
		for (int i = 0; i < keynamesArray.Length; ++i) {
			try {
				if (Input.GetKeyDown (keynamesArray [i]))
					return true;
			} catch {
				;
			}
	...
	public void UpdateInternal (bool editorMode, float mousePositionX, float mousePositionY, bool editDig, bool editInverse)
	{
		...
		if (!editorMode) {
			if (!instanceReady) {
				prepareInstance ();
			} else if (Time.time > instanceCheckNext)
				checkInstance ();
		} else {
			checkInstance ();
		}
		...
		if (!editorMode) {
			if (layerRenderWater)
				fluid_stat_countA = fluid_stat_countB = fluid_stat_count = fluid_stat_count0 = fluid_stat_count1 = fluid_stat_count2 = 0;
			if (Time.time > waterPassNext) {
				if (layerRenderWater && (layerWaterReference != null))
					for (int i = 0; i < waterPasses; ++i)
						fluid_blob (layerWaterReference, true);
				waterPassNext += waterPassLapse;
				if (Time.time > waterPassNext + 10f)
					waterPassNext = Time.time + waterPassLapse;
			}
		}
		if (layerRenderWater) {
		...
		if (Time.time > editNext) {
			if (editInRuntimeEnabled || spawnEnabled)
				for (int i = 0; i < editPasses; ++i)
					editInstance ();
			editNext += editLapse;
			if (Time.time > editNext + 10f)
				editNext = Time.time + editLapse;
		}
		if (!editorMode)
		if (saveEnabled)
		if (keynamesGetKeyDown (saveKeynames))
			saveLayer ("layer." + saveRecordName + ".txt");
		if (!editorMode)
		if (cameraHold)
		if (keynamesGetKeyDown (cameraHoldKeynames))
			cameraHoldActive = !cameraHoldActive;
		if (!editorMode)
		if (keynamesGetKeyDown (specialKeynames))
			outlineLayer ();
		if (!editorMode)
		if (cameraHold)
			cameraHoldInstance ();
	...
	void prepareInstance ()
	{
		...
		if (dataPerBlockHorizontal > 0f)
			ddWidth = dataWidth / dataPerBlockHorizontal;
		if (dataPerBlockVertical > 0f)
			ddHeight = dataHeight / dataPerBlockVertical;
		...
		if (saveAutoLoadFile)
			loadLayer ("layer." + saveRecordName + ".txt");
		checkInstance ();
	}
	...
	void checkInstance ()
	{
		...
		if ((neighbourXmYm != null) && (neighbourXmYm.neighbourXpYp == null))
			neighbourXmYm.neighbourXpYp = this;

		if ((neighbourYm != null) && (neighbourYm.neighbourYp == null))
			neighbourYm.neighbourYp = this;
		if ((neighbourYm != null) && (neighbourXmYm != null) && (neighbourYm.neighbourXm == null))
			neighbourYm.neighbourXm = neighbourXmYm;
		if ((neighbourYm != null) && (neighbourXpYm != null) && (neighbourYm.neighbourXp == null))
			neighbourYm.neighbourXp = neighbourXpYm;
		if ((neighbourXmYm != null) && (neighbourYm != null) && (neighbourXmYm.neighbourXp == null))
			neighbourXmYm.neighbourXp = neighbourYm;
		if ((neighbourXpYm != null) && (neighbourYm != null) && (neighbourXpYm.neighbourXm == null))
			neighbourXpYm.neighbourXm = neighbourYm;
		if ((neighbourXm != null) && (neighbourYm != null) && (neighbourXm.neighbourXpYm == null))
			neighbourXm.neighbourXpYm = neighbourYm;
		if ((neighbourXp != null) && (neighbourYm != null) && (neighbourXp.neighbourXmYm == null))
			neighbourXp.neighbourXmYm = neighbourYm;

		if ((neighbourXpYm != null) && (neighbourXpYm.neighbourXmYp == null))
			neighbourXpYm.neighbourXmYp = this;

		if ((neighbourXm != null) && (neighbourXm.neighbourXp == null))
			neighbourXm.neighbourXp = this;
		if ((neighbourXm != null) && (neighbourXmYm != null) && (neighbourXm.neighbourYm == null))
			neighbourXm.neighbourYm = neighbourXmYm;
		if ((neighbourXm != null) && (neighbourXmYp != null) && (neighbourXm.neighbourYp == null))
			neighbourXm.neighbourYp = neighbourXmYp;
		if ((neighbourXmYm != null) && (neighbourXm != null) && (neighbourXmYm.neighbourYp == null))
			neighbourXmYm.neighbourYp = neighbourXm;
		if ((neighbourXmYp != null) && (neighbourXm != null) && (neighbourXmYp.neighbourYm == null))
			neighbourXmYp.neighbourYm = neighbourXm;
		if ((neighbourYm != null) && (neighbourXm != null) && (neighbourYm.neighbourXmYp == null))
			neighbourYm.neighbourXmYp = neighbourXm;
		if ((neighbourYp != null) && (neighbourXm != null) && (neighbourYp.neighbourXmYm == null))
			neighbourYp.neighbourXmYm = neighbourXm;

		if ((neighbourXp != null) && (neighbourXp.neighbourXm == null))
			neighbourXp.neighbourXm = this;
		if ((neighbourXp != null) && (neighbourXpYm != null) && (neighbourXp.neighbourYm == null))
			neighbourXp.neighbourYm = neighbourXpYm;
		if ((neighbourXp != null) && (neighbourXpYp != null) && (neighbourXp.neighbourYp == null))
			neighbourXp.neighbourYp = neighbourXpYp;
		if ((neighbourXpYm != null) && (neighbourXp != null) && (neighbourXpYm.neighbourYp == null))
			neighbourXpYm.neighbourYp = neighbourXp;
		if ((neighbourXpYp != null) && (neighbourXp != null) && (neighbourXpYp.neighbourYm == null))
			neighbourXpYp.neighbourYm = neighbourXp;
		if ((neighbourYm != null) && (neighbourXp != null) && (neighbourYm.neighbourXpYp == null))
			neighbourYm.neighbourXpYp = neighbourXp;
		if ((neighbourYp != null) && (neighbourXp != null) && (neighbourYp.neighbourXpYm == null))
			neighbourYp.neighbourXpYm = neighbourXp;

		if ((neighbourXmYp != null) && (neighbourXmYp.neighbourXpYm == null))
			neighbourXmYp.neighbourXpYm = this;

		if ((neighbourYp != null) && (neighbourYp.neighbourYm == null))
			neighbourYp.neighbourYm = this;
		if ((neighbourYp != null) && (neighbourXmYp != null) && (neighbourYp.neighbourXm == null))
			neighbourYp.neighbourXm = neighbourXmYp;
		if ((neighbourYp != null) && (neighbourXpYp != null) && (neighbourYp.neighbourXp == null))
			neighbourYp.neighbourXp = neighbourXpYp;
		if ((neighbourXmYp != null) && (neighbourYp != null) && (neighbourXmYp.neighbourXp == null))
			neighbourXmYp.neighbourXp = neighbourYp;
		if ((neighbourXpYp != null) && (neighbourYp != null) && (neighbourXpYp.neighbourXm == null))
			neighbourXpYp.neighbourXm = neighbourYp;
		if ((neighbourXm != null) && (neighbourYp != null) && (neighbourXm.neighbourXpYp == null))
			neighbourXm.neighbourXpYp = neighbourYp;
		if ((neighbourXp != null) && (neighbourYp != null) && (neighbourXp.neighbourXmYp == null))
			neighbourXp.neighbourXmYp = neighbourYp;

		if ((neighbourXpYp != null) && (neighbourXpYp.neighbourXmYm == null))
			neighbourXpYp.neighbourXmYm = this;

		if (layerMaster != null) {
			if (layerMaster.layerSlave == null)
				layerMaster.layerSlave = this;
			else if (layerMaster.layerSlave != this)
			if (debugMessages)
				Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": " + gameObject.name + " layerMaster (" + layerMaster.gameObject.name + ") has other layerSlave: " + layerMaster.layerSlave.gameObject.name + " but each layerMaster can have only 1 layerSlave.");
			if (!layerSlaveReady)
				dumpLayer ();
		}
		if (layerSlave != null) {
			if (layerSlave.layerMaster == null)
				layerSlave.layerMaster = this;
			else if (layerSlave.layerMaster != this)
			if (debugMessages)
				Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": " + gameObject.name + " layerSlave (" + layerSlave.gameObject.name + ") has other layerMaster: " + layerSlave.layerMaster.gameObject.name + " but each layerSlave can have only 1 layerMaster.");
		}
		...
		if (layerMaster == null) {
			...
			if (debugMessages)
				Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ":layerSlave: " + ((layerSlave == null) ? "null" : layerSlave.ToString ()));
			if (layerSlave != null)
			if (debugMessages)
				Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ":layerSlave.data: " + ((layerSlave.data == null) ? "null" : layerSlave.data.ToString ()));
		...
		if (data != null) {
			if (debugMessages)
				Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": " + lastDataWidth + " x " + lastDataHeight + " data cells");
			...
			checkInstanceTiling ();
			if (debugMessages)
				Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": " + blocksWidth + " x " + blocksHeight + " blocks");
			if ((blocks == null) || (blocks.Length != blocksWidth * blocksHeight)) {
				...
				if (debugMessages)
					Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": creating new blocks");
				TerrainBlock2D[] blocks1 = new TerrainBlock2D[blocksWidth * blocksHeight];
				if (debugMessages)
					Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": before " + lastBlocksWidth + " x " + lastBlocksHeight + ", now " + blocksWidth + " x " + blocksHeight + " blocks");
				for (int i = 0; i < lastBlocksWidth; ++i)
					for (int j = 0; j < lastBlocksHeight; ++j) {
						if ((i >= blocksWidth) || (j >= blocksHeight)) {
							if ((blocks [i + j * lastBlocksWidth] != null) && (blocks [i + j * lastBlocksWidth].gameObject != null)) {
								if (debugMessages)
									Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": destroy GameObject: " + blocks [i + j * lastBlocksWidth].gameObject.name);
				...
				for (int i = 0; i < blocksWidth; ++i)
					for (int j = 0; j < blocksHeight; ++j) {
						if ((i < lastBlocksWidth) && (j < lastBlocksHeight))
							blocks1 [i + j * blocksWidth] = blocks [i + j * lastBlocksWidth];
						else {
						...
						if (blocks1 [i + j * blocksWidth].gameObject != null) {
							...
							block1.width = lastDataWidth - block1.left;
							if (block1.width > dataPerBlockHorizontal)
								block1.width = dataPerBlockHorizontal;
							block1.height = lastDataHeight - block1.top;
							if (block1.height > dataPerBlockVertical)
								block1.height = dataPerBlockVertical;
							block1.meshMetadata = new MeshMetadata ();
							if (layerCollideWalls)
								block1.colliderMesh = new Mesh ();
							if (layerCollideWalls)
								block1.colliderMeshMetadata = new MeshMetadata ();
						}
				...
				for (int i = 0; i < blocksWidth; ++i)
					for (int j = 0; j < blocksHeight; ++j) {
						if (blocks [i + j * blocksWidth] != null) {
							if ((i > 0) && (j > 0))
							if (blocks [(i - 1) + (j - 1) * blocksWidth] != null)
								blocks [i + j * blocksWidth].XmYm = blocks [(i - 1) + (j - 1) * blocksWidth];
							if ((i > 0) && (j < blocksHeight - 1))
							if (blocks [(i - 1) + (j + 1) * blocksWidth] != null)
								blocks [i + j * blocksWidth].XmYp = blocks [(i - 1) + (j + 1) * blocksWidth];
							if ((i < blocksWidth - 1) && (j > 0))
							if (blocks [(i + 1) + (j - 1) * blocksWidth] != null)
								blocks [i + j * blocksWidth].XpYm = blocks [(i + 1) + (j - 1) * blocksWidth];
							if ((i < blocksWidth - 1) && (j < blocksHeight - 1))
							if (blocks [(i + 1) + (j + 1) * blocksWidth] != null)
								blocks [i + j * blocksWidth].XpYp = blocks [(i + 1) + (j + 1) * blocksWidth];
							if ((i > 0))
							if (blocks [(i - 1) + (j) * blocksWidth] != null)
								blocks [i + j * blocksWidth].Xm = blocks [(i - 1) + (j) * blocksWidth];
							if ((j > 0))
							if (blocks [(i) + (j - 1) * blocksWidth] != null)
								blocks [i + j * blocksWidth].Ym = blocks [(i) + (j - 1) * blocksWidth];
							if ((i < blocksWidth - 1))
							if (blocks [(i + 1) + (j) * blocksWidth] != null)
								blocks [i + j * blocksWidth].Xp = blocks [(i + 1) + (j) * blocksWidth];
							if ((j < blocksHeight - 1))
							if (blocks [(i) + (j + 1) * blocksWidth] != null)
								blocks [i + j * blocksWidth].Yp = blocks [(i) + (j + 1) * blocksWidth];
						}
					}
	...
	bool blockAllocGameObject (int i, int j, TerrainBlock2D block, string name, bool layerCollideWalls, bool attachToAllocList)
	{
		...
		if (block.gameObject != null) {
			...
			block.width = lastDataWidth - block.left;
			if (block.width > dataPerBlockHorizontal)
				block.width = dataPerBlockHorizontal;
			block.height = lastDataHeight - block.top;
			if (block.height > dataPerBlockVertical)
				block.height = dataPerBlockVertical;
			block.meshMetadata = new MeshMetadata ();
			if (layerCollideWalls)
				block.colliderMesh = new Mesh ();
			if (layerCollideWalls)
				block.colliderMeshMetadata = new MeshMetadata ();
			return true;
		}
		return false;
	}
	...
	void checkInstanceData ()
	{
		if ((data == null) || (data.Length != dataWidth * dataHeight) || (data.Length != lastDataWidth * lastDataHeight)) {
			...
			for (int i = 0; i < dataWidth; ++i)
				for (int j = 0; j < dataHeight; ++j) {
					if ((i < lastDataWidth) && (j < lastDataHeight))
						data1 [i + j * dataWidth] = data [i + j * lastDataWidth];
					else
						data1 [i + j * dataWidth] = layerDefaultFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
				}
			data = data1;
			lastDataWidth = dataWidth;
			lastDataHeight = dataHeight;
		...
		if ((datad == null) || (datad.Length != ddWidth * ddHeight * dataMaterialLayers) || (datad.Length != lastDatadWidth * lastDatadHeight * lastDataMaterialLayers)) {
			...
			if (debugMessages)
				Debug.Log ("Terrain2D: checkInstance: " + gameObject.name + ": creating new density data");
			float[] datad1 = new float[ddWidth * ddHeight * dataMaterialLayers];
			for (int i = 0; i < ddWidth; ++i)
				for (int j = 0; j < ddHeight; ++j)
					for (int n = 0; n < dataMaterialLayers; ++n) {
						if ((i < lastDatadWidth) && (j < lastDatadHeight) && (n < lastDataMaterialLayers))
							datad1 [i + j * ddWidth + n * ddWidth * ddHeight] = datad [i + j * lastDatadWidth + n * lastDatadWidth * lastDatadHeight];
						else
							datad1 [i + j * ddWidth + n * ddWidth * ddHeight] = layerDefaultFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
					}
			datad = datad1;
			lastDatadWidth = ddWidth;
			lastDatadHeight = ddHeight;
	...
	public void outlineLayer ()
	{
		switch (editorLayer) {
		default:
		case TerrainLayer2D.master:
			dataLayer = -1;
			break;
		case TerrainLayer2D.material1:
			dataLayer = 0;
			break;
		case TerrainLayer2D.material2:
			dataLayer = 1;
			break;
		case TerrainLayer2D.material3:
			dataLayer = 2;
			break;
		case TerrainLayer2D.material4:
			dataLayer = 3;
			break;
		}
		...
		if ((array_internal_draw_li_cache1 != null) && (array_internal_draw_li_cache2 != null)) {
			...
			for (int i = 0; (i < array_internal_draw_li_cache1.Length) && (i < array_internal_draw_li_cache2.Length) && (i < count_internal_draw_li_cache); ++i)
				internal_outlineShapesCount = i + 1;
	...
	public Vector3[] outlineLayerShape (int i, bool recalculate)
	{
		if ((internal_outlineShapes != null) && (i >= 0) && (i < internal_outlineShapes.Length)) {
			if (recalculate) {
				for (int j = 0; j < internal_outlineShapes[i].shape.Length; ++j)
					internal_outlineShapes [i].shape [j] = gameObject.transform.position + gameObject.transform.right * internal_outlineShapes [i].shape2d [j].x + gameObject.transform.up * internal_outlineShapes [i].shape2d [j].y;
			}
	...
	public void saveLayer (string filename)
	{
		...
		try {
			try {
				...
				sw.Close ();
				if (debugMessages)
					Debug.Log (gameObject.name + ": saveLayer: Could not save file " + fichname + " because " + e.Message);
			}
		} catch (Exception e) {
			if (debugMessages)
				Debug.Log (gameObject.name + ": saveLayer: Could not save file " + fichname + " because " + e.Message);
		...
		try {
			try {
				...
				do {
					...
					if (max_count < 0) {
						if (debugMessages)
							Debug.Log (gameObject.name + ": loadLayer: Could not read from file " + fichname + " and aborting because of an unknown error (after reading " + data_count + " values and " + data_lines + " lines)");
						return;
					}
							...
							if (keyname.Equals ("class")) {
								...
								if ((keyname.Length >= 3) && keyname.Substring (0, 3).Equals ("row")) {
									...
									if (data_row >= 0) {
										...
										if (data_version_ok)
											data_rows = parseInt (keyvalue, -1);
									}
								} else if (keyname.Equals ("cols")) {
									if (data_version_ok)
										data_cols = parseInt (keyvalue, -1);
									...
									if (data_version >= 0) {
										...
										if (!data_version_ok)
										if (debugMessages)
											Debug.Log (gameObject.name + ": loadLayer: version is old (object version = " + dataVersion + ", file version = " + data_version + ")");
									}
								} else {
									if (debugMessages)
										Debug.Log (gameObject.name + ": loadLayer: unknown keyword: '" + keyname + "'");
									sr.Close ();
									return;
								}
							}
				...
				if (data_version_ok)
					Debug.Log (gameObject.name + ": loadLayer: Readed file " + fichname + ", " + data_count + " terrain2d values had been loaded.");
				else
					Debug.Log (gameObject.name + ": loadLayer: The stored file is older than currently loaded, no data has been updated.");
			} catch (Exception e) {
				sr.Close ();
				if (debugMessages)
					Debug.Log (gameObject.name + ": loadLayer: Could not read from file " + fichname + " because " + e.Message);
			}
		} catch (Exception e) {
			if (debugMessages)
				Debug.Log (gameObject.name + ": loadLayer: Could not read from file " + fichname + " because " + e.Message);
	...
	{
		if (!editorMode)
			cameraHoldDistance -= Input.GetAxis ("Mouse ScrollWheel") * cameraHoldDistance / 100f * 85.0f;
		if (cameraHoldDistance < 0f)
			cameraHoldDistance = 0f;
		if (cameraHoldDistance > 100f)
			cameraHoldDistance = 100f;
		if (cameraManagement)
		if (cameraHoldActive)
			Camera.main.transform.position = cameraPosition = cameraPosition * (1f - cameraPositionFilter) + cameraPositionFilter * (gameObject.transform.position - gameObject.transform.forward * cameraHoldDistance + mousePositionX / Screen.width * gameObject.transform.right * lastDataWidth / scale.x + mousePositionY / Screen.height * gameObject.transform.up * lastDataHeight / scale.y);
	}
		...
		for (int i = 0; i < blocksWidth; ++i)
			for (int j = 0; j < blocksHeight; ++j) {
				if (blocks [i + j * blocksWidth].dirty) {
					blockRedraw (blocks [i + j * blocksWidth]);
					if ((layerSlave != null) && (layerSlave.blocks != null) && (layerSlave.blocksWidth == blocksWidth) && (blocks.Length == layerSlave.blocks.Length) && (i < layerSlave.blocksWidth) && (j < layerSlave.blocksHeight))
						layerSlave.blockRedraw (layerSlave.blocks [i + j * blocksWidth]);
	...
	{
		if (dataLayer == -1) {
			if ((data != null) && (i >= 0) && (j >= 0) && (i < lastDataWidth) && (j < lastDataHeight))
				return data [i + j * lastDataWidth];
		} else {
			if (dataPerBlockHorizontal > 0f)
				i = i / dataPerBlockHorizontal;
			if (dataPerBlockVertical > 0f)
				j = j / dataPerBlockVertical;
			if ((datad != null) && (i >= 0) && (j >= 0) && (dataLayer >= 0) && (i < lastDatadWidth) && (j < lastDatadHeight) && (dataLayer < lastDataMaterialLayers))
				return datad [i + j * lastDatadWidth + dataLayer * lastDatadWidth * lastDatadHeight];
		}
	...
	float dataWrite (int i, int j, float newValue)
	{
		if (dataLayer == -1) {
			if ((data != null) && (i >= 0) && (j >= 0) && (i < lastDataWidth) && (j < lastDataHeight))
				return data [i + j * lastDataWidth] = newValue;
		} else {
			if (dataPerBlockHorizontal > 0f)
				i = i / dataPerBlockHorizontal;
			if (dataPerBlockVertical > 0f)
				j = j / dataPerBlockVertical;
			if ((datad != null) && (i >= 0) && (j >= 0) && (dataLayer >= 0) && (i < lastDatadWidth) && (j < lastDatadHeight) && (dataLayer < lastDataMaterialLayers))
				return datad [i + j * lastDatadWidth + dataLayer * lastDatadWidth * lastDatadHeight] = newValue;
		}
		return layerDefaultFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
	}
	...
	void blockRedrawBorderRedraw (TerrainBlock2D block)
	{
		...
		float tmp;
		if ((block.outlineConnectedVertex1 != null) && (block.outlineExistsConnectedVertex1 != null) && (block.outlineConnectedVertex1.Length == block.outlineExistsConnectedVertex1.Length) && (block.outlineConnectedVertex1.Length == block.outlineVertex1.Length))
			outlineExistsConnectedVertex1 = true;
		if ((block.outlineConnectedVertex2 != null) && (block.outlineExistsConnectedVertex2 != null) && (block.outlineConnectedVertex2.Length == block.outlineExistsConnectedVertex2.Length) && (block.outlineConnectedVertex2.Length == block.outlineVertex2.Length))
			outlineExistsConnectedVertex2 = true;
	...
	{
		if (Time.time < block.lastRedrawTime + blockRedraw_minLapse)
			return;
		...
		if ((block.mesh = bindMesh (block.gameObject, useMaterial, false)) != null) {
			...
			if (layerRenderBorder) {
				...
				if ((block.outlineVertex1 != null) && (block.outlineVertex2 != null) && (block.outlineVertex1.Length == block.outlineVertex2.Length)) {
					if (block.outlineConnectedVertex1 == null)
						block.outlineConnectedVertex1 = new Vector2[block.outlineVertex1.Length];
					if (block.outlineConnectedVertex2 == null)
						block.outlineConnectedVertex2 = new Vector2[block.outlineVertex2.Length];
					if (block.outlineExistsConnectedVertex1 == null) {
						block.outlineExistsConnectedVertex1 = new bool[block.outlineVertex1.Length];
						for (int i = 0; i < block.outlineVertex1.Length; ++i)
							block.outlineExistsConnectedVertex1 [i] = false;
					}
					if (block.outlineExistsConnectedVertex2 == null) {
						block.outlineExistsConnectedVertex2 = new bool[block.outlineVertex2.Length];
						for (int i = 0; i < block.outlineVertex2.Length; ++i)
							block.outlineExistsConnectedVertex2 [i] = false;
					}
					if ((block.outlineConnectedVertex1 != null) && (block.outlineConnectedVertex2 != null) && (block.outlineConnectedVertex1.Length == block.outlineConnectedVertex2.Length)) {
						if ((block.outlineExistsConnectedVertex1 != null) && (block.outlineExistsConnectedVertex2 != null) && (block.outlineExistsConnectedVertex1.Length == block.outlineExistsConnectedVertex2.Length)) {
							if (block.XmYm != null)
								blockRedrawBorderScan (block, block.XmYm, false);
							if (block.Ym != null)
								blockRedrawBorderScan (block, block.Ym, false);
							if (block.XpYm != null)
								blockRedrawBorderScan (block, block.XpYm, false);
							if (block.Xm != null)
								blockRedrawBorderScan (block, block.Xm, false);
							if (block.Xp != null)
								blockRedrawBorderScan (block, block.Xp, false);
							if (block.XmYp != null)
								blockRedrawBorderScan (block, block.XmYp, false);
							if (block.Yp != null)
								blockRedrawBorderScan (block, block.Yp, false);
							if (block.XpYp != null)
								blockRedrawBorderScan (block, block.XpYp, false);
			...
			if (layerCollideWalls)
				meshDrawClear (block.colliderMesh, block.colliderMeshMetadata);
			if (layerRenderEmpty) {
				ofsz = zEmptyOffset + 0f;
				redraw_blob3 (block, true, false, false, false, false, false);
			}
			if (layerRenderFilled) {
				ofsz = zOffset + 0f;
				redraw_blob3 (block, false, true, false, false, false, false);
			}
			if (layerRenderWalls) {
				ofsz = zWallsOffset + 0f;
				ofszw = zWallsWidth;
				redraw_blob3 (block, false, false, true, false, false, false);
			}
			...
			if (layerRenderBorder) {
				ofsz = zBorderOffset + 0f;
				ofszw = zBorderWidth;
				internal_draw_li_dry_init ();
				redraw_blob3 (block, false, false, true, false, false, true);
				internal_draw_li_dry_joinborders ();
				internal_draw_li_dry_renderborders ();
				internal_draw_li_dry_release ();
			}
#endif
			if (layerCollideWalls) {
				ofsz = zWallsOffset + 0f;
				ofszw = zWallsWidth;
				redraw_blob3 (block, false, false, true, false, true, false);
			}
#if TERRAIN_2D_PHYSICS
			if (layerCollideWalls2D) {
				...
				if (block.gameObject != null) {
					...
					if ((array_internal_draw_li_altcache1 != null) && (array_internal_draw_li_altcache2 != null)) {
						int internal_outlineShapesCount = 0;
						for (int i = 0; (i < array_internal_draw_li_altcache1.Length) && (i < array_internal_draw_li_altcache2.Length) && (i < count_internal_draw_li_altcache); ++i)
							internal_outlineShapesCount = i + 1;

						if (internal_outlineShapesCount > 0) {
							if ((collider2d = (PolygonCollider2D)block.gameObject.GetComponent ("PolygonCollider2D")) == null)
			...
			if (layerRenderWater) {
				ofsz = zOffset + 0f;
				redraw_blob1 (block, true, false, false, false, false);
			}
	...
	public TerrainBrushType2D blockPlotBrushtype (float iradius, float idistance, TerrainBrushType2D ibrushtype)
	{
		...
		case TerrainBrushType2D.distance_single_or_multi_cell_auto:
			if (idistance >= 50f)
				ibrushtype = TerrainBrushType2D.multi_cell_soft;
			else if (idistance > 25f)
				ibrushtype = TerrainBrushType2D.single_cell_soft;
			else
				ibrushtype = TerrainBrushType2D.single_cell_hard;
			break;
		case TerrainBrushType2D.smooth_distance_single_or_multi_cell_auto:
			if (idistance >= 50f)
				ibrushtype = TerrainBrushType2D.smooth_multi_cell_soft;
			else if (idistance > 25f)
				ibrushtype = TerrainBrushType2D.smooth_single_cell_soft;
			else
				ibrushtype = TerrainBrushType2D.smooth_single_cell_hard;
			break;
		}
		return ibrushtype;
	}
	...
	public bool blockDataAtNeighbour (TerrainBlock2D block, int ii, int jj, int i, int j, float delta, out float value)
	{
		...
		if ((ii < 0) && (jj < 0)) {
			if (neighbourXmYm != null) {
				...
				if ((ii + jj * neighbourXmYm.blocksWidth >= 0) && (ii + jj * neighbourXmYm.blocksWidth < neighbourXmYm.blocks.Length))
					newblock = neighbourXmYm.blocks [ii + jj * neighbourXmYm.blocksWidth];
				else
					return false;
			...
			if (neighbourXmYp != null) {
				...
				jj -= blocksHeight;
				if ((ii + jj * neighbourXmYp.blocksWidth >= 0) && (ii + jj * neighbourXmYp.blocksWidth < neighbourXmYp.blocks.Length))
					newblock = neighbourXmYp.blocks [ii + jj * neighbourXmYp.blocksWidth];
				else
					return false;
			...
			if (neighbourXpYm != null) {
				...
				if ((ii + jj * neighbourXpYm.blocksWidth >= 0) && (ii + jj * neighbourXpYm.blocksWidth < neighbourXpYm.blocks.Length))
					newblock = neighbourXpYm.blocks [ii + jj * neighbourXpYm.blocksWidth];
				else
					return false;
			...
			if (neighbourXpYp != null) {
				ii -= blocksWidth;
				jj -= blocksHeight;
				if ((ii + jj * neighbourXpYp.blocksWidth >= 0) && (ii + jj * neighbourXpYp.blocksWidth < neighbourXpYp.blocks.Length))
					newblock = neighbourXpYp.blocks [ii + jj * neighbourXpYp.blocksWidth];
				else
					return false;
			...
			if (neighbourXm != null) {
				...
				if ((ii + jj * neighbourXm.blocksWidth >= 0) && (ii + jj * neighbourXm.blocksWidth < neighbourXm.blocks.Length))
					newblock = neighbourXm.blocks [ii + jj * neighbourXm.blocksWidth];
				else
					return false;
			...
			if (neighbourXp != null) {
				ii -= blocksWidth;
				if ((ii + jj * neighbourXp.blocksWidth >= 0) && (ii + jj * neighbourXp.blocksWidth < neighbourXp.blocks.Length))
					newblock = neighbourXp.blocks [ii + jj * neighbourXp.blocksWidth];
				else
					return false;
			...
			if (neighbourYm != null) {
				...
				if ((ii + jj * neighbourYm.blocksWidth >= 0) && (ii + jj * neighbourYm.blocksWidth < neighbourYm.blocks.Length))
					newblock = neighbourYm.blocks [ii + jj * neighbourYm.blocksWidth];
				else
					return false;
			...
			if (neighbourYp != null) {
				jj -= blocksHeight;
				if ((ii + jj * neighbourYp.blocksWidth >= 0) && (ii + jj * neighbourYp.blocksWidth < neighbourYp.blocks.Length))
					newblock = neighbourYp.blocks [ii + jj * neighbourYp.blocksWidth];
				else
					return false;
		...
		if ((newlayer == null) || (newblock == null))
			return false;
		if (ipending)
			i += newblock.width;
		if (jpending)
	...
	public float blockData (TerrainBlock2D block, int i, int j, float delta, bool noDirtyMark)
	{
		...
		if ((i >= 0) && (j >= 0) && (i < block.width) && (j < block.height)) {
			...
			if (block.Ym != null) {
				...
				if ((blockofs >= 0) && (blockofs < neighbourYm.blocks.Length))
					return neighbourYm.blockData (neighbourYm.blocks [blockofs], i, j + neighbourYm.blocks [blockofs].height, delta);
				else
					return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
			} else {
				return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
			...
			if (block.Xm != null) {
				...
				if ((blockofs >= 0) && (blockofs < neighbourXm.blocks.Length))
					return neighbourXm.blockData (neighbourXm.blocks [blockofs], i + neighbourXm.blocks [blockofs].width, j, delta);
				else
					return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
			} else {
				return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
			...
			if (block.Yp != null) {
				...
				if ((blockofs >= 0) && (blockofs < neighbourYp.blocks.Length))
					return neighbourYp.blockData (neighbourYp.blocks [blockofs], i, j - block.height, delta);
				else
					return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
			} else {
				return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
			...
			if (block.Xp != null) {
				...
				if ((blockofs >= 0) && (blockofs < neighbourXp.blocks.Length))
					return neighbourXp.blockData (neighbourXp.blocks [blockofs], i - block.width, j, delta);
				else
					return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
			} else {
				return layerDefaultOutsideFilled ? DEFAULT_DATA : (1f - DEFAULT_DATA);
	...
	protected void debugflags ()
	{
		...
		if (cerop)
			cero = !cero;
		if (unop || dosp || tresp || cuatrop)
			Debug.Log ("redraw with uno = " + uno + " y dos = " + dos + " y tres = " + tres + " y cuatro = " + cuatro);
	}
	...
	[System.NonSerialized]
	public bool enable_internal_draw_li_cache = false;
	[System.NonSerialized]
	public Vector2[] array_internal_draw_li_cache1 = null;
	[System.NonSerialized]
	public Vector2[] array_internal_draw_li_cache2 = null;
	[System.NonSerialized]
	public int count_internal_draw_li_cache = 0;
	[System.NonSerialized]
	private int maxcount_internal_draw_li_cache = 9999;

	[System.NonSerialized]
	public bool enable_internal_draw_li_altcache = false;
	[System.NonSerialized]
	public Vector2[] array_internal_draw_li_altcache1 = null;
	[System.NonSerialized]
	public Vector2[] array_internal_draw_li_altcache2 = null;
	[System.NonSerialized]
	public int count_internal_draw_li_altcache = 0;
	[System.NonSerialized]
	private int maxcount_internal_draw_li_altcache = 9999;
	...
	{
		if (internal_draw_li_dry_block == null)
			internal_draw_li_dry_block = new TerrainBlock2D[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_x1 == null)
			internal_draw_li_dry_x1 = new float[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_y1 == null)
			internal_draw_li_dry_y1 = new float[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_x2 == null)
			internal_draw_li_dry_x2 = new float[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_y2 == null)
			internal_draw_li_dry_y2 = new float[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_link11 == null)
			internal_draw_li_dry_link11 = new int[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_link12 == null)
			internal_draw_li_dry_link12 = new int[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_link21 == null)
			internal_draw_li_dry_link21 = new int[internal_draw_li_dry_maxcount];
		if (internal_draw_li_dry_link22 == null)
			internal_draw_li_dry_link22 = new int[internal_draw_li_dry_maxcount];
		internal_draw_li_dry_count = 0;
		if (internal_draw_li_dry_blocks == null)
			internal_draw_li_dry_blocks = new TerrainBlock2D[internal_draw_li_dry_maxcount];
	...
	{
		for (int i = 0; i < internal_draw_li_dry_blocks_count; ++i)
			internal_draw_li_dry_blocks [i] = null;
		internal_draw_li_dry_blocks_count = 0;
	}
	...
	protected void internal_draw_li_dry_renderborders (TerrainBlock2D block, TerrainBlock2D inblock, float dx, float dy, float dz, bool auto)
	{
		...
		for (int i = 0; i < internal_draw_li_dry_count; ++i)
			if ((inblock == null) || (inblock == internal_draw_li_dry_block [i])) {
				if (block == null)
					internal_tiling_set (internal_draw_li_dry_block [i]);
		...
		for (int i = 0; i < internal_draw_li_dry_count; ++i) {
			if (internal_draw_li_dry_block [i] != null) {
				if (i > j) {
					...
					if (internal_draw_li_dry_link11 [i] >= 0)
						internal_draw_li_dry_link11 [internal_draw_li_dry_link11 [i]] = j;
					internal_draw_li_dry_link11 [j] = internal_draw_li_dry_link11 [i];
					if (internal_draw_li_dry_link12 [i] >= 0)
						internal_draw_li_dry_link21 [internal_draw_li_dry_link12 [i]] = j;
					internal_draw_li_dry_link12 [j] = internal_draw_li_dry_link12 [i];
					if (internal_draw_li_dry_link21 [i] >= 0)
						internal_draw_li_dry_link12 [internal_draw_li_dry_link21 [i]] = j;
					internal_draw_li_dry_link21 [j] = internal_draw_li_dry_link21 [i];
					if (internal_draw_li_dry_link22 [i] >= 0)
						internal_draw_li_dry_link22 [internal_draw_li_dry_link22 [i]] = j;
	...
	{
		for (int i = 0; i < internal_draw_li_dry_count; ++i)
			if (internal_draw_li_dry_block [i] == block) {
				if (internal_draw_li_dry_link11 [i] >= 0)
					internal_draw_li_dry_link11 [internal_draw_li_dry_link11 [i]] = -1;
				if (internal_draw_li_dry_link11 [i] >= 0)
					internal_draw_li_dry_link11 [i] = -1;
				if (internal_draw_li_dry_link12 [i] >= 0)
					internal_draw_li_dry_link21 [internal_draw_li_dry_link12 [i]] = -1;
				if (internal_draw_li_dry_link12 [i] >= 0)
					internal_draw_li_dry_link12 [i] = -1;
				if (internal_draw_li_dry_link21 [i] >= 0)
					internal_draw_li_dry_link12 [internal_draw_li_dry_link21 [i]] = -1;
				if (internal_draw_li_dry_link21 [i] >= 0)
					internal_draw_li_dry_link21 [i] = -1;
				if (internal_draw_li_dry_link22 [i] >= 0)
					internal_draw_li_dry_link22 [internal_draw_li_dry_link22 [i]] = -1;
				if (internal_draw_li_dry_link22 [i] >= 0)
					internal_draw_li_dry_link22 [i] = -1;
				internal_draw_li_dry_block [i] = null;
			}
	...
	protected void internal_draw_li (TerrainBlock2D block, float x1, float y1, float x2, float y2, string color, float width)
	{
		...
		if (internal_draw_li_dry) {
			...
			if (array_internal_draw_li_cache1 == null)
				array_internal_draw_li_cache1 = new Vector2[maxcount_internal_draw_li_cache];
			if (array_internal_draw_li_cache2 == null)
				array_internal_draw_li_cache2 = new Vector2[maxcount_internal_draw_li_cache];
			if (count_internal_draw_li_cache < array_internal_draw_li_cache1.Length)
				array_internal_draw_li_cache1 [count_internal_draw_li_cache] = new Vector2 (x1, y1);
			if (count_internal_draw_li_cache < array_internal_draw_li_cache2.Length)
				array_internal_draw_li_cache2 [count_internal_draw_li_cache] = new Vector2 (x2, y2);
			++count_internal_draw_li_cache;
		} else if (enable_internal_draw_li_altcache) {
			if (array_internal_draw_li_altcache1 == null)
				array_internal_draw_li_altcache1 = new Vector2[maxcount_internal_draw_li_altcache];
			if (array_internal_draw_li_altcache2 == null)
				array_internal_draw_li_altcache2 = new Vector2[maxcount_internal_draw_li_altcache];
			if (count_internal_draw_li_altcache < array_internal_draw_li_altcache1.Length)
				array_internal_draw_li_altcache1 [count_internal_draw_li_altcache] = new Vector2 (x1, y1);
			if (count_internal_draw_li_altcache < array_internal_draw_li_altcache2.Length)
				array_internal_draw_li_altcache2 [count_internal_draw_li_altcache] = new Vector2 (x2, y2);
	...
	{
		if (uno)
			internal_draw_li (block, x1, y1, x2, y2, color, width);
	}
	...
	protected void redraw_blob0 (TerrainBlock2D block, bool drawpositive, bool drawnegative, bool drawborder, bool drawdebug)
	{
		...
		for (int blobx = 0; blobx < block.width; ++blobx) {
			for (int bloby = 0; bloby < block.height; ++bloby) {
				...
				if (drawpositive) {
					NWx = 0.5f - 0.5f * valueC;
					NWy = 0.5f - 0.5f * valueC;
					NEx = 0.5f + 0.5f * valueC;
					NEy = 0.5f - 0.5f * valueC;
					SWx = 0.5f - 0.5f * valueC;
					SWy = 0.5f + 0.5f * valueC;
					SEx = 0.5f + 0.5f * valueC;
					SEy = 0.5f + 0.5f * valueC;
					draw_quad (
						block,
	...
	protected void redraw_blob1 (TerrainBlock2D block, bool drawpositive, bool drawnegative, bool drawborder, bool drawdebug, bool meshcollider)
	{
		...
		for (int blobx = 0; blobx < block.width; ++blobx) {
			for (int bloby = 0; bloby < block.height; ++bloby) {
				valueC = blockData (block, blobx, bloby, 0, 0, 0.0f);
				if (blobDstep)
					valueC = Mathf.RoundToInt (valueC / tStep) * tStep;
				biasx = 0.5f;
				biasy = 0.5f;
				deltax = 0.25f;
				deltay = 0.25f;
				...
				if (valueC >= 1.0f - threshold) {
					...
					fN = valueN > Fthreshold;
					fW = valueW > Fthreshold;
					fE = valueE > Fthreshold;
					fS = valueS > Fthreshold;
					if (fN && fW && fE) {
						fS = false;
						if (valueS > Fthreshold2)
							fS = true;
					}
					...
					if (valueC < threshold_droplets) {
						if (UnityEngine.Random.value > probability_droplets * (valueC - threshold) / (threshold_droplets - threshold))
							NN = false;
						if (UnityEngine.Random.value > probability_droplets * (valueC - threshold) / (threshold_droplets - threshold))
							WW = false;
						if (UnityEngine.Random.value > probability_droplets * (valueC - threshold) / (threshold_droplets - threshold))
					...
					NWx = fW ? 0.0f + biasx : 1.0f - biasx - (valueN) * deltax;
					NWy = fN ? 0.0f + biasy : 1.0f - biasy - (valueW) * deltay;
					NEx = fE ? 1.0f - biasx : 0.0f + biasx + (valueN) * deltax;
					NEy = fN ? 0.0f + biasy : 1.0f - biasy - (valueE) * deltay;
					SWx = fW ? 0.0f + biasx : 1.0f - biasx - (valueS) * deltax;
					SWy = (fS || (!fN)) ? 1.0f - biasy : 0.0f + biasy + (valueW) * deltay;
					SEx = fE ? 1.0f - biasx : 0.0f + biasx + (valueS) * deltax;
					SEy = (fS || (!fN)) ? 1.0f - biasy : 0.0f + biasy + (valueE) * deltay;
					if (NWx > NEx) {
						NWx = (NWx + NEx - threshold * 0.1f) * 0.5f;
						NEx = (NWx + NEx + threshold * 0.1f) * 0.5f;
						Ncolor = Ecolor = Wcolor = Scolor = "green";
					}
					if (SWx > SEx) {
						SWx = (SWx + SEx - threshold * 0.1f) * 0.5f;
						SEx = (SWx + SEx + threshold * 0.1f) * 0.5f;
						Ncolor = Ecolor = Wcolor = Scolor = "red";
					}
					CCx = (NWx + NEx + SWx + SEx) / 4;
					CCy = (NWy + NEy + SWy + SEy) / 4;
				}
		...
		for (int blobx = 0; blobx < block.width; ++blobx) {
			for (int bloby = 0; bloby < block.height; ++bloby) {
				...
				if (valueC <= 0.0f) {
					...
					valueNW = blockData (block, blobx, bloby, -1, -1, 0.0f);
					if (blobDstep)
						valueNW = Mathf.RoundToInt (valueNW / tStep) * tStep;
					valueN = blockData (block, blobx, bloby, 0, -1, 0.0f);
					if (blobDstep)
						valueN = Mathf.RoundToInt (valueN / tStep) * tStep;
					valueNE = blockData (block, blobx, bloby, +1, -1, 0.0f);
					if (blobDstep)
						valueNE = Mathf.RoundToInt (valueNE / tStep) * tStep;
					valueW = blockData (block, blobx, bloby, -1, 0, 0.0f);
					if (blobDstep)
						valueW = Mathf.RoundToInt (valueW / tStep) * tStep;
					valueE = blockData (block, blobx, bloby, +1, 0, 0.0f);
					if (blobDstep)
						valueE = Mathf.RoundToInt (valueE / tStep) * tStep;
					valueSW = blockData (block, blobx, bloby, -1, +1, 0.0f);
					if (blobDstep)
						valueSW = Mathf.RoundToInt (valueSW / tStep) * tStep;
					valueS = blockData (block, blobx, bloby, 0, +1, 0.0f);
					if (blobDstep)
						valueS = Mathf.RoundToInt (valueS / tStep) * tStep;
					valueSE = blockData (block, blobx, bloby, +1, +1, 0.0f);
					if (blobDstep)
						valueSE = Mathf.RoundToInt (valueSE / tStep) * tStep;
					...
					if (valueN <= tMin) {
						if (valueNW <= tMin)
							NWy = 1.0f - (valueC + valueW) * 0.5f * EEamp;
						if (valueNE <= tMin)
							NEy = 1.0f - (valueC + valueE) * 0.5f * EEamp;
					}
					if (valueW <= tMin) {
						if (valueNW <= tMin)
							NWx = 1.0f - (valueC + valueN) * 0.5f * EEamp;
						if (valueSW <= tMin)
							SWx = 1.0f - (valueC + valueS) * 0.5f * EEamp;
					}
					if (valueE <= tMin) {
						if (valueNE <= tMin)
							NEx = (valueC + valueN) * 0.5f * EEamp;
						if (valueSE <= tMin)
							SEx = (valueC + valueS) * 0.5f * EEamp;
					}
					if (valueS <= tMin) {
						if (valueSW <= tMin)
							SWy = (valueC + valueW) * 0.5f * EEamp;
						if (valueSE <= tMin)
							SEy = (valueC + valueE) * 0.5f * EEamp;
					}

					if (NWx > NEx) {
						NWx = NEx = (NWx + NEx) / 2.0f;
					}
					if (SWx > SEx) {
						SWx = SEx = (SWx + SEx) / 2.0f;
					}
					if (NWy > SWy) {
						NWy = SWy = (NWy + SWy) / 2.0f;
					}
					if (NEy > SEy) {
						NEy = SEy = (NEy + SEy) / 2.0f;
					}
					...
					if (drawborder) {
						...
						if (valueC > tMax) {
							fN = Mathf.Abs (valueN - 0.0f) < threshold;
							fW = Mathf.Abs (valueW - 0.0f) < threshold;
							fE = Mathf.Abs (valueE - 0.0f) < threshold;
							fS = Mathf.Abs (valueS - 0.0f) < threshold;
							fNW = Mathf.Abs (valueNW - 0.0f) < threshold;
							fNE = Mathf.Abs (valueNE - 0.0f) < threshold;
							fSW = Mathf.Abs (valueSW - 0.0f) < threshold;
							fSE = Mathf.Abs (valueSE - 0.0f) < threshold;
							fNWe = fN && fNW && fW;
							fNEe = fN && fNE && fE;
							fSWe = fS && fSW && fW;
							fSEe = fS && fSE && fE;
	...
	protected float fluidto_reference (int x, int y, Terrain2D referencelayer, bool referencenegative)
	{
		...
		if (referencenegative) {
			if ((referencelayer == null) || (referencelayer.data [Mathf.FloorToInt (x * referencelayer.dataWidth / dataWidth) + referencelayer.dataWidth * Mathf.FloorToInt (y * referencelayer.dataHeight / dataHeight)] < reference_threshold)) {
				...
				reference_max_fill = reference_multiplier * referencelayer.data [Mathf.FloorToInt (x * referencelayer.dataWidth / dataWidth) + referencelayer.dataWidth * Mathf.FloorToInt (y * referencelayer.dataHeight / dataHeight)];
		} else {
			if ((referencelayer == null) || (referencelayer.data [Mathf.FloorToInt (x * referencelayer.dataWidth / dataWidth) + referencelayer.dataWidth * Mathf.FloorToInt (y * referencelayer.dataHeight / dataHeight)] > 1f - reference_threshold)) {
				...
				reference_max_fill = reference_multiplier * (1f - referencelayer.data [Mathf.FloorToInt (x * referencelayer.dataWidth / dataWidth) + referencelayer.dataWidth * Mathf.FloorToInt (y * referencelayer.dataHeight / dataHeight)]);
		}
		if (reference_max_fill > 1f)
			reference_max_fill = 1f;
	...
	protected bool fluidto_blob (int ix, int iy, int tx, int ty, float ammount, bool balancing, Terrain2D referencelayer, bool referencenegative, bool deltaammount)
	{
		...
		if (referencelayer)
			reference_max_fill = fluidto_reference (tx, ty, referencelayer, referencenegative);
		if (reference_max_fill < -99f)
			return false;
		if (reference_max_fill > tMax)
			reference_max_fill = 1f;
		++fluid_stat_countB;
	...
	protected void fluid_blob (Terrain2D referencelayer, bool referencenegative)
	{
		...
		case 0:
			++fluid_stat_count2;
			for (var bloby = 1; bloby <= dataHeight - 2; ++bloby)
				for (var blobx = 1; blobx <= dataWidth - 2; ++blobx)
					if (data [blobx + dataWidth * bloby] > 0.0f)
						fluid_iteration (blobx, bloby, referencelayer, referencenegative);
			break;
		case 1:
			++fluid_stat_count2;
			for (var bloby = 1; bloby <= dataHeight - 2; ++bloby)
				for (var blobx = dataWidth - 2; blobx >= 0; --blobx)
					if (data [blobx + dataWidth * bloby] > 0.0f)
						fluid_iteration (blobx, bloby, referencelayer, referencenegative);
			break;
		case 2:
			++fluid_stat_count2;
			for (var bloby = dataHeight - 2; bloby >= 0; ++bloby)
				for (var blobx = 1; blobx <= dataWidth - 2; ++blobx)
					if (data [blobx + dataWidth * bloby] > 0.0f)
						fluid_iteration (blobx, bloby, referencelayer, referencenegative);
			break;
	...
	protected Mesh bindMesh (MeshMetadata meta, GameObject gameObject, string shaderName, bool materialOnlyIfNotSet)
	{
		...
		if (autoReady)
			meta.ready = true;
		if (meta != null)
			return meta.mesh = bindMeshInternal (gameObject);
		else
			return bindMeshInternal (gameObject);
	}
	protected Mesh bindMesh (MeshMetadata meta, GameObject gameObject, Material material, bool materialOnlyIfNotSet)
	{
		...
		if (material != null)
			bindMeshRenderer (meta, gameObject, material, materialOnlyIfNotSet);
		else
			bindMeshRendererFromShaderName (gameObject, DEFAULT_SHADER, materialOnlyIfNotSet);
	...
	protected bool meshDrawClear (MeshMetadata meta)
	{
		...
		if ((meta != null) && (meta.mesh != null))
			return meshDrawClear (meta.mesh, meta);
		return false;
	}
	...
	{
		for (int i = 0; i < meta.usedVertices; ++i)
			if (((v - meta.vertices [i]).magnitude < threshold) && ((n - meta.normals [i]).magnitude < nthreshold) && ((uv - meta.uvs [i]).magnitude < uvthreshold))
				return i;
		return -1;
	}
	...
	{
		for (int i = 0; i < meta.usedVertices; ++i)
			if (((v - meta.vertices [i]).magnitude < threshold) && ((uv - meta.uvs [i]).magnitude < uvthreshold))
				return i;
		return -1;
	}
	...
	{
		for (int i = 0; i < meta.usedVertices; ++i)
			if (((v - meta.vertices [i]).magnitude < threshold) && ((n - meta.normals [i]).magnitude < nthreshold))
				return i;
		return -1;
	}
	...
	{
		for (int i = 0; i < meta.usedVertices; ++i)
			if ((v - meta.vertices [i]).magnitude < threshold)
				return i;
		return -1;
	}
	...
	{
		if ((meta != null) && (meta.mesh != null))
			return meshImmediate (meta.mesh, meta, applyVertices, applyNormals, applyUV, applyTriangles);
		return false;
	}
	...
	{
		if ((meta != null) && (meta.mesh != null))
			return meshImmediate (meta.mesh, meta, true, true, true, true, true);
		return false;
	}
	protected bool meshImmediate ()
	{
		if ((defaultMeshMetadata != null) && (defaultMeshMetadata.mesh != null))
			return meshImmediate (defaultMeshMetadata.mesh, defaultMeshMetadata, true, true, true, true, true);
		return false;
	}
	protected bool meshImmediate (Mesh mesh, MeshMetadata meta, bool applyVertices, bool applyNormals, bool applyUV, bool applyTriangles, bool backupTriangles2)
	{
		...
		if (applyTriangles && (meta.triangles != null) && (meta.triangles2 != null) && (meta.usedTriangles2 == meta.usedTriangles)) {
			for (int i = 0; i < meta.usedTriangles; ++i) {
				if (meta.triangles [i] != meta.triangles2 [i])
					break;
				if (i == meta.usedTriangles - 1)
					refreshTriangles = false;
			}
		}
		...
		if (refreshTriangles && applyTriangles && (meta.triangles != null)) {
			if (meta.triangles.Length > meta.usedTriangles) {
				...
				for (int i = 0; i < meta.usedTriangles; ++i)
					tmpTriangles [i] = meta.triangles [i];
	...
	protected bool meshDrawTriangle (Mesh mesh, MeshMetadata meta, Vector3 v1, Vector3 v2, Vector3 v3, bool applyNormals, Vector3 n1, Vector3 n2, Vector3 n3, bool applyUVs, Vector2 uv1, Vector2 uv2, Vector2 uv3, float threshold, bool immediate)
	{
		...
		if ((v1 - v2).magnitude < threshold)
			return false;
		if ((v1 - v3).magnitude < threshold)
			return false;
		if ((v2 - v3).magnitude < threshold)
			return false;

		int i1, i2, i3, oi = 0;
		if (meta.vertices == null)
			meta.vertices = new Vector3[immediate ? (meta.usedVertices) : (meta.usedVertices * 2 + 16)];
		if (applyNormals || meta.autogenerateNormals)
		if (meta.normals == null)
			meta.normals = new Vector3[immediate ? (meta.usedVertices) : (meta.usedVertices * 2 + 16)];
		if (applyUVs || meta.autogenerateUVs)
		if (meta.uvs == null)
			meta.uvs = new Vector2[immediate ? (meta.usedVertices) : (meta.usedVertices * 2 + 16)];
		...
		if (meta.autogenerateUVs && (!applyUVs)) {
			...
			++autouv_log_count;
			if (autouv_log_count % autouv_log_rate == 0) {
			}
		}
		...
		if (i1 < 0) {
			if (autoxyz_transform) {
				...
				if (applyNormals || meta.autogenerateNormals)
					meta.normals [i1] = n1.x * autoxyz_right + n1.y * autoxyz_up + n1.z * autoxyz_forward;
			} else {
		...
		if (i2 < 0) {
			if (autoxyz_transform) {
				...
				if (applyNormals || meta.autogenerateNormals)
					meta.normals [i2] = n2.x * autoxyz_right + n2.y * autoxyz_up + n2.z * autoxyz_forward;
			} else {
		...
		if (i3 < 0) {
			if (autoxyz_transform) {
				...
				if (applyNormals || meta.autogenerateNormals)
					meta.normals [i3] = n3.x * autoxyz_right + n3.y * autoxyz_up + n3.z * autoxyz_forward;
			} else {
		...
		if (meta.triangles == null)
			meta.triangles = new int[immediate ? (meta.usedTriangles) : (meta.usedTriangles * 3 + 18)];
		...
		if (immediate)
			return meshImmediate (mesh, meta, true, applyNormals || meta.autogenerateNormals, applyUVs || meta.autogenerateUVs, true);
		else
			return true;
	}
